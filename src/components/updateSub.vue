      ...
      <div class="row justify-content-center">
        <div class="col-md-6">
          <template v-if="childDataLoaded">
            <submission-form :sub="sub" subbtnt="Update Submission"
                              @submission-is-created-updated="updateSubmission"></submission-form>
          </template>
        </div><!-- /col -->
      </div><!-- /row -->
    </div><!-- /container -->
...
export default {
  data () {
    return {
      ...
      temp: {},
      messagetitle: ' Update Submission '
    }
  },
  components: {
    ...
    getsubmission: function () {
      SubmissionServices.findOneSubmission(this.$router.params)
        .then(response => {
          ...
          console.log('getting the sub' + JSON.stringify(this.sub, null, 5))
        })
        .catch(error => {
